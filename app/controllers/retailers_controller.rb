class RetailersController < ApplicationController
  ...
  def show
    ...
    @ordersPending = Order.where(shopper_address: Redis.current.get("address"), status: '0')
    @ordersCompleted = Order.where(shopper_address: Redis.current.get("address"), status: '1')
  end
  ...
  private
    ...
    def set_retailer
      ...
      Redis.current.set("retailer_id","#{@retailer.id}")
      puts "============== REDIS SAVED RETAILER ID: #{@retailer.id} ==============="
      @categories = Category.all
      @cart = Cart.where(shopper_id: Redis.current.get("shopper_id"))
      if(@cart[0] == nil)
        @prods = "Cart is empty"
    ...
    def retailer_params
      params.require(:retailer).permit(:name, :email, :pass, :phone, :address)
    end
end
